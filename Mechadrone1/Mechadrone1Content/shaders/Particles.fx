                               ...
                               float age, float normalizedAge)
{
    ...
    return mul(float4(position, 1), gViewProj);
}
...
float2x2 Create2DRotation(float rotation)
{
...
{    
    ...
    return Create2DRotation(rotation);
}


// Custom vertex shader animates particles entirely on the GPU.
VertexShaderOutput StandardParticleVertexShader(VertexShaderInput input)
{
    ...
    output.Color.a = atan(-(30.0f * normalizedAge - 30.0f)) / PI * 2.0f + 0.02f;
    output.TextureCoordinate = -0.5f * input.Corner + float2(0.5f, 0.5f);

    return output;
}

float3 ComputeVelocity(float3 initialVelocity, float age, float normalizedAge)
{
    float3 noGravVelocity = initialVelocity + initialVelocity * EndVelocity * normalizedAge;
    return noGravVelocity + 2.0f * Gravity * normalizedAge;
}


// Custom vertex shader animates particles entirely on the GPU.
VertexShaderOutput ShowerParticleVertexShader(VertexShaderInput input)
{
    VertexShaderOutput output;
    ...
    float3 wVelocity = ComputeVelocity(input.Velocity, age, normalizedAge);
    float4 hVelocity = mul(float4(wVelocity, 0.0f), gViewProj);
    hVelocity /= hVelocity.w;
    float velocityTheta = atan2(-hVelocity.x, hVelocity.y);

    float2x2 rotation = Create2DRotation(velocityTheta);

    output.Position.xy += mul(input.Corner, rotation) * size * ViewportScale;
    output.Color = lerp(MinColor, MaxColor, input.Random.z);
    output.Color.a = atan(-(30.0f * normalizedAge - 30.0f)) / PI * 2.0f + 0.02f;
    output.TextureCoordinate = -0.5f * input.Corner + float2(0.5f, 0.5f);
...
// Standard technique for drawing particles.
technique StandardParticles
{
    pass P0
    {
        VertexShader = compile vs_2_0 StandardParticleVertexShader();
        PixelShader = compile ps_2_0 ParticlePixelShader();
    }
}

// Particles where rotation matches velocity
technique ShowerParticles
{
    pass P0
    {
        VertexShader = compile vs_2_0 ShowerParticleVertexShader();
        PixelShader = compile ps_2_0 ParticlePixelShader();
    }
}
